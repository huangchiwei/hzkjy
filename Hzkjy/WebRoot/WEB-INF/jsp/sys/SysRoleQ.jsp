	...
	function delConfirm(roleNo){
		$.layer({
		  ...
		  dialog : {
		      ...
		      btn : ['是','否'],
		      yes : function(){
		          location.href='${ctx}/sys/sysRole/delete/' + roleNo + '.html';
		      },
		      no : function(index){
		         layer.close(index);
		      }
...
<div class="btn_box">
     ...
     <input <hc:attribute attrName="disabled" permValue="ro_save"/> id="add_bt" type="button" value="添加" class="initial" style="cursor:hand" onclick="javascript:location.href='${ctx}/sys/sysRole/add/new.html'"/>
  	
  </div>
  <div class="list_info">
  	<form action="${ctx}/sys/sysRole/list/1.html" method="post">
    <h2>按条件查询</h2>
    <div class="div2">
    ...
    <table width="98%" border="1" cellpadding="0" cellspacing="0">
	  ...
	  <!-- 变量 -->
	  	<pm:hasPermission permValue="ro_saveperm">
	       	<c:set var="ro_saveperm" value="true"/>
	    </pm:hasPermission>
	    <pm:hasPermission permValue="ro_updt">
	       	<c:set var="ro_updt" value="true"/>
	    </pm:hasPermission>
	    <pm:hasPermission permValue="ro_del">
	       	<c:set var="ro_del" value="true"/>
	    </pm:hasPermission>
		      ...
		      <tr>
		        ...
		        <td>
		         	<fmt:formatDate value="${role.modifyDate}"
									pattern="yyyy-MM-dd" /></td>
		        <td>
		        	  <c:if test="${ro_saveperm == true}">
				          <div class="btn_icon">
				          	<input type="image" src="${ctx}/theme/default/images/allocation.png" title="分配权限" onclick="javascript:location.href='${ctx}/sys/sysRole/toRoleToPerm.html?roleNo=${role.roleNo}'"/>
				          </div>
			          </c:if>
			          <c:if test="${ro_updt == true}">
				          <div class="btn_icon">
				          	<input type="image" src="${ctx}/theme/default/images/edit_icon.png" title="修改" onclick="javascript:location.href='${ctx}/sys/sysRole/add/new.html?roleNo=${role.roleNo}'"/>
				          </div>
			          </c:if>
			          <c:if test="${ro_del == true}">
				          <div class="btn_icon">
				          	<input type="image" src="${ctx}/theme/default/images/del_icon.png" title="删除" onclick="delConfirm('${role.roleNo}')"/>
				          </div> 
			          </c:if>
		         </td>
