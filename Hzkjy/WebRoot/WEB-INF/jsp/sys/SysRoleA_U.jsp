	...
	$(function(){
		...
		$("#roleNo").blur(function(){
			if (/^\w{6,20}$/.test($.trim(this.value))){
				// 发送同步请求
				$.ajax({
					url : "${ctx}/sys/sysRole/validRoleNo.html?random=" + Math.random(),
					type : "post",
...
<form id="add_form" action='<c:if test="${role == null}">${ctx}/sys/sysRole/save.html</c:if><c:if test="${role != null}">${ctx}/sys/sysRole/update/${role.roleNo}.html</c:if>' method="post" >
  <div class="add_info">
   ...
   <c:choose>
    	<c:when test="${role == null}">
    		<pm:hasPermission permValue="ro_save">
    			<p class="div_submit">
