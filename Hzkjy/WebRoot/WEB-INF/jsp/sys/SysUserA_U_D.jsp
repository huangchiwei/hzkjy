	...
	$(function(){
		...
		$("#userNo").blur(function(){
			if (/^\w{6,20}$/.test($.trim(this.value))){
				// 发送同步请求
				$.ajax({
					url : "${ctx}/sys/sysUser/validUserNo.html?random=" + Math.random(),
					type : "post",
...
<form id="add_form" action='<c:if test="${user == null}">${ctx}/sys/sysUser/save.html</c:if><c:if test="${user != null and update != null}">${ctx}/sys/sysUser/update/${user.userNo}.html</c:if>' method="post">
