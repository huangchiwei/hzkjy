package com.armysoft.hzkjy.model;

import java.util.Date;

public class MemberBasic implements java.io.Serializable {
	
	private Integer zlid;
	public Integer getZlid() {
		return zlid;
	}
	public void setZlid(Integer zlid) {
		this.zlid = zlid;
	}
	public Integer getKjid() {
		return kjid;
	}
	public void setKjid(Integer kjid) {
		this.kjid = kjid;
	}
	private Integer kjid;
	private Integer id;
	private String hybh;
	private String qymc;
	private String qymcpy;
	private String address;
	private String zydy;
	private String mj;
	private String frdb;
	private String zczb;
	private String lxr;
	private String zt;
	private Date qyrzsj;
	private Date htqxf;
	private Date htqxe;
	private String qydjzclx;
    private String lxrdh;
	private String sfjjyb;
	private String zzjgdm;
	private String frlxdh;
	private Date qyclsj;
	private String htstze;
	private String dzys;
    private String xnyjdxs;
	private String qyssjsly;
	private String gxjsqy;
	private String lxryqy;
    private String hylb;
	private String synyxzscq;
	private String fmzl;
	private String wgsj;
	private String syxx;
	private String rjzzq;
	private String ztName;
	private String sfjjybName;
	public String getZtName() {
		return ztName;
	}
	public void setZtName(String ztName) {
		this.ztName = ztName;
	}
	public String getSfjjybName() {
		return sfjjybName;
	}
	public void setSfjjybName(String sfjjybName) {
		this.sfjjybName = sfjjybName;
	}
	public Integer getId() {
		return id;
	}
	public void setId(Integer id) {
		this.id = id;
	}
	public String getHybh() {
		return hybh;
	}
	public void setHybh(String hybh) {
		this.hybh = hybh;
	}
	public String getQymc() {
		return qymc;
	}
	public void setQymc(String qymc) {
		this.qymc = qymc;
	}
	public String getQymcpy() {
		return qymcpy;
	}
	public void setQymcpy(String qymcpy) {
		this.qymcpy = qymcpy;
	}
	public String getAddress() {
		return address;
	}
	public void setAddress(String address) {
		this.address = address;
	}
	public String getZydy() {
		return zydy;
	}
	public void setZydy(String zydy) {
		this.zydy = zydy;
	}
	public String getMj() {
		return mj;
	}
	public void setMj(String mj) {
		this.mj = mj;
	}
	public String getFrdb() {
		return frdb;
	}
	public void setFrdb(String frdb) {
		this.frdb = frdb;
	}
	public String getZczb() {
		return zczb;
	}
	public void setZczb(String zczb) {
		this.zczb = zczb;
	}
	public String getLxr() {
		return lxr;
	}
	public void setLxr(String lxr) {
		this.lxr = lxr;
	}
	public String getZt() {
		return zt;
	}
	public void setZt(String zt) {
		this.zt = zt;
	}
	public Date getQyrzsj() {
		return qyrzsj;
	}
	public void setQyrzsj(Date qyrzsj) {
		this.qyrzsj = qyrzsj;
	}
	public Date getHtqxf() {
		return htqxf;
	}
	public void setHtqxf(Date htqxf) {
		this.htqxf = htqxf;
	}
	public Date getHtqxe() {
		return htqxe;
	}
	public void setHtqxe(Date htqxe) {
		this.htqxe = htqxe;
	}
	public String getQydjzclx() {
		return qydjzclx;
	}
	public void setQydjzclx(String qydjzclx) {
		this.qydjzclx = qydjzclx;
	}
	public String getLxrdh() {
		return lxrdh;
	}
	public void setLxrdh(String lxrdh) {
		this.lxrdh = lxrdh;
	}
	public String getSfjjyb() {
		return sfjjyb;
	}
	public void setSfjjyb(String sfjjyb) {
		this.sfjjyb = sfjjyb;
	}
	public String getZzjgdm() {
		return zzjgdm;
	}
	public void setZzjgdm(String zzjgdm) {
		this.zzjgdm = zzjgdm;
	}
	public String getFrlxdh() {
		return frlxdh;
	}
	public void setFrlxdh(String frlxdh) {
		this.frlxdh = frlxdh;
	}
	public Date getQyclsj() {
		return qyclsj;
	}
	public void setQyclsj(Date qyclsj) {
		this.qyclsj = qyclsj;
	}
	public String getHtstze() {
		return htstze;
	}
	public void setHtstze(String htstze) {
		this.htstze = htstze;
	}
	public String getDzys() {
		return dzys;
	}
	public void setDzys(String dzys) {
		this.dzys = dzys;
	}
	public String getXnyjdxs() {
		return xnyjdxs;
	}
	public void setXnyjdxs(String xnyjdxs) {
		this.xnyjdxs = xnyjdxs;
	}
	public String getQyssjsly() {
		return qyssjsly;
	}
	public void setQyssjsly(String qyssjsly) {
		this.qyssjsly = qyssjsly;
	}
	public String getGxjsqy() {
		return gxjsqy;
	}
	public void setGxjsqy(String gxjsqy) {
		this.gxjsqy = gxjsqy;
	}
	public String getLxryqy() {
		return lxryqy;
	}
	public void setLxryqy(String lxryqy) {
		this.lxryqy = lxryqy;
	}
	public String getHylb() {
		return hylb;
	}
	public void setHylb(String hylb) {
		this.hylb = hylb;
	}
	public String getSynyxzscq() {
		return synyxzscq;
	}
	public void setSynyxzscq(String synyxzscq) {
		this.synyxzscq = synyxzscq;
	}
	public String getFmzl() {
		return fmzl;
	}
	public void setFmzl(String fmzl) {
		this.fmzl = fmzl;
	}
	public String getWgsj() {
		return wgsj;
	}
	public void setWgsj(String wgsj) {
		this.wgsj = wgsj;
	}
	public String getSyxx() {
		return syxx;
	}
	public void setSyxx(String syxx) {
		this.syxx = syxx;
	}
	public String getRjzzq() {
		return rjzzq;
	}
	public void setRjzzq(String rjzzq) {
		this.rjzzq = rjzzq;
	}
}